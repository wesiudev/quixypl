...
export default function SecondProductEdit({
  source,
  place,
}: {
  source: any;
  ...
  function updateAutomatically() {
    setLoading(true);
    if (place === "products") {
      updateSecondProduct(product.id, product).then(() => {
        setLoading(false), setSavedAutomatically(true);
      });
    } else if (place === "drafts") {
      updateSecondDraft(product.id, product).then(() => {
        setLoading(false), setSavedAutomatically(true);
      });
    } else if (place === "new") {
      if (draftCreated) {
        setLoading(true);
        updateSecondDraft(product.id, product).then(() => setLoading(false));
      }
    }
  }
  useEffect(() => {
    ...
    if (!draftCreated && place === "new")
      createSecondDraft({ ...product, id: id }, id).then(() => {
        setLoading(false);
      });
  ...
  return (
    <div
      className={`bg-black w-full duration-500 ${
              ...
              {place !== "new" && (
                <>
                  <button
                    onClick={() => {
                      if (place === "products") {
                        deleteSecondProduct(product.id).then(() =>
                          router.push("/admin/products")
                        );
                      } else if (place === "drafts") {
                        deleteSecondDraft(product.id).then(() =>
                  ...
                  <button
                    onClick={() => {
                      if (
                        ...
                        deleteSecondDraft(product?.id).then(() =>
                          createSecondProduct(product).then(() =>
                            router.push("/admin/products")
                          )
                        );
                      } else {
