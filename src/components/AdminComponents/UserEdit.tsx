...
export default function UserEdit({
  ...
  function updateAutomatically() {
    setLoading(true);
    if (place === "products") {
      updateUserProduct(product.id, product).then(() => {
        setLoading(false), setSavedAutomatically(true);
      });
    } else if (place === "drafts") {
      updateUserDraft(product.id, product).then(() => {
        setLoading(false), setSavedAutomatically(true);
      });
    } else if (place === "new") {
      if (draftCreated) {
        setLoading(true);
        updateUserDraft(freshId, product).then(() => setLoading(false));
      }
    }
  }
  useEffect(() => {
    ...
    if (!draftCreated && place === "new")
      createUserDraft({ ...product, id: id }, id).then(() => {
        setFreshId(id);
        setLoading(false);
      });
              ...
              {place !== "new" && (
                <>
                  <button
                    onClick={() => {
                      if (place === "products") {
                        deleteUserProduct(product.id).then(() =>
                          router.push("/admin/products")
                        );
                      } else if (place === "drafts") {
                        deleteUserDraft(product.id).then(() =>
                    ...
                    <button
                      onClick={() => {
                        if (
                          ...
                          deleteUserDraft(product.id).then(() =>
                            createUserProduct(product).then(() =>
                              router.push("/admin/products")
                            )
                          );
                        } else {
