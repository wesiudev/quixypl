...
export default async function Page({
  ...
  return (
    <>
          ...
          <div className="items-end flex justify-between w-full text-sm bg-white !text-black relative z-50">
            <div className="flex flex-col breadcrumbs">
              <ul className="flex items-center flex-wrap">
                <li className="">
                  <Link href={`/company`} title="praca zdalna talent">
                    company
                  </Link>
                </li>
                <li className="">
          ...
          <div>
            <div className="mt-6">
                  ...
                  <div className="flex">
                    {slug?.photoURL && (
                      <Image
                        ...
                        height={256}
                        alt=""
                        className="rounded-l-xl mb-0 w-40"
                      />
                    )}
                    {!slug?.photoURL && (
                      <div className="bg-[#126b91] h-full rounded-l-xl aspect-square text-white flex items-center justify-center w-40">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 mt-3">
                <div className="mr-1">
                  <h2 className="w-max px-2 py-1 rounded-lg text-xl text-white bg-gradient-to-r from-primary to-cta drop-shadow-lg font-gotham mt-3">
                    Specjalizacje
                  </h2>
      ...
      <div className="container mx-auto py-12 bg-white px-4 sm:px-0">
            ...
            <div className="">
              <TalentList
                categoryTalents={talents.filter((item: any) =>
                  item?.tags?.find((tag: any) => tag.slugUrl === slug?.slugUrl)
                )}
              />
            </div>
          </div>
...
export async function generateMetadata({ params }: { params: any }) {
  const title = `Quixy Talent™ ${params.slug} | Zatrudnij do Pracy Zdalnej`;
  const description = `Prowadzisz rekrutację lub szukasz pracy zdalnej? Sprawdź projekty ${params.slug} i współpracujcie gdziekolwiek jesteście, zdalnie`;
